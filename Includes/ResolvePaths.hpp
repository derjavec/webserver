...
{      
    public:
        static const LocationConfig* findBestLocation(Server &server, const std::string &path, size_t &bestMatchLength);
        ...
        static bool findLocationConfig(Server &server, const std::string &path, std::string &locationIndex, bool &autoindexEnabled, std::string &filePath, const std::string &url);
        static bool matchLocationPath(const std::string &url, const std::string &locationPath);
        static bool isMethodAllowed(Server &server, const std::string &path, const std::string &method, const std::string &url);
